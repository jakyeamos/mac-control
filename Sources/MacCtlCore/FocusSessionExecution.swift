...
/// Converts AppKit's global bottom-left screen coordinates into the global
/// top-left coordinate space used by Accessibility window frames.
public enum FocusSessionLayout {
    public static func targetFrames(
        visibleFrame: CGRect,
        primaryScreenMaxY: CGFloat
    ) -> (brief: CGRect, scratchpad: CGRect) {
        let gutter: CGFloat = 12
        let columnWidth = (visibleFrame.width - gutter) / 2
        let accessibilityY = primaryScreenMaxY - visibleFrame.maxY
        return (
            CGRect(
                x: visibleFrame.minX,
                y: accessibilityY,
            ...
            CGRect(
                x: visibleFrame.minX + columnWidth + gutter,
                y: accessibilityY,
                width: columnWidth,
                height: visibleFrame.height
            )
...
public protocol FocusSessionActionExecuting {
    func execute(_ operation: FocusSessionExecutionOperation) throws -> FocusSessionExecutionResult
    func verify(_ operation: FocusSessionExecutionOperation) throws -> Bool
}
...
public final class SystemFocusSessionActionExecutor: FocusSessionActionExecuting {
    ...
    public func execute(_ operation: FocusSessionExecutionOperation) throws -> FocusSessionExecutionResult {
        switch operation {
        case .openBrief:
    ...
    public func verify(_ operation: FocusSessionExecutionOperation) throws -> Bool {
        switch operation {
        case .openBrief:
            let fixture = try materializeBrief()
        ...
        case .arrangeWorkspace:
            ...
            let targets = targetFrames(screen: screen)
            ...
            return approximatelyEqual(brief, targets.brief)
                && approximatelyEqual(scratchpad, targets.scratchpad)
        }
    }
    ...
    private func arrangeWorkspace() throws {
        guard let screen = NSScreen.main else {
            throw FocusSessionExecutionError.layoutVerificationFailed
        }
        let targets = targetFrames(screen: screen)
        let briefPID = try runningPID(bundleID: FocusSessionVerifier.previewBundleID)
        let scratchpadPID = try runningPID(bundleID: FocusSessionVerifier.textEditBundleID)
    ...
    private func targetFrames(screen: NSScreen) -> (brief: CGRect, scratchpad: CGRect) {
        let primaryScreenMaxY = NSScreen.screens.first?.frame.maxY ?? screen.frame.maxY
        return FocusSessionLayout.targetFrames(
